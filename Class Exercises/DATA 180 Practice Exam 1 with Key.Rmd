...
departments[departments == "Marketing"] <- "Sales and Marketing"
```
...
```{r}
# Plot the bar chart
barplot(height = rbind(sales_data$Product_A, sales_data$Product_B), beside = TRUE,
        names.arg = sales_data$Quarter, legend = c("Product A", "Product B"),
        main = "Total Sales of Product A and Product B by Quarter", ylab = "Sales ($)")
```
...
quarter_summary <- function(sales_data) {
  ...
  return(list(Average_Product_A = avg_product_a, Average_Product_B = avg_product_b))
}
...
hard_courses <- student_courses$Course[course_difficulty == "Hard"]
students_in_hard_courses <- student_courses$Student[student_courses$Course %in% hard_courses]
students_in_hard_courses
```
